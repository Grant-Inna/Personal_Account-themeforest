  ...
  bundle: function(bundle) {
    ...
    if (typeof bundle.src !== 'undefined' && typeof bundle.bundle !== 'undefined') {
      ...
      for (var type in bundle.src) {
        ...
        switch (type) {
          case 'styles':
            gulp.src(bundle.src[type]).pipe(_self.cssRewriter(bundle.bundle[type])()).pipe(concat(outputFile)).pipe(sass({
              errLogToConsole: true,
              includePaths: includePaths,
            }).on('error', sass.logError)).pipe(_self.cssChannel()()).pipe(_self.outputChannel(bundle.bundle[type], outputFile)());
            break;
  ...
  output: function(bundle) {
    ...
    if (typeof bundle.src !== 'undefined' && typeof bundle.output !== 'undefined') {
      for (var type in bundle.src) {
        ...
        switch (type) {
          case 'styles':
            gulp.src(bundle.src[type]).pipe(sass({
              errLogToConsole: true,
              includePaths: includePaths,
            }).on('error', sass.logError)).pipe(_self.outputChannel(bundle.output[type])());
            break;
          case 'scripts':
